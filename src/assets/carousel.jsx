    ...
    return (
        <>
            ...
            <div id={`carousel-${tabId}`} className="carousel-item">
                <div className="md:flex bg-cover pt-12 md:pt-28 justify-center md:gap-3" style={{ backgroundImage: `url(${background})`, opacity: "0.8"}}>
                    <div className="flex flex-col items-center align-bottom">
                        ...
                        <button type="button" 
                                className="border border-stone-800 my-4 md:my-10 px-10 md:px-16 py-1 md:py-2 text-stone-900 hover:bg-stone-100 shadow-[4px_4px_8px_rgba(0,0,0,0.2)] text-lg md:text-2xl"
                                onClick={() => handleOpen()}>{opened ? "Close box" : "Open Box"}</button>
                    </div>
                    <div className="px-8 w-full max-w-lg ">
                        <h3 className={`text-5xl md:text-6xl ${text_color} [text-shadow:_5px_5px_8px_rgb(0_0_0_/_20%)] md:py-[5%]`}>{callout}</h3>
                        <div className="flex flex-row items-center gap-4 pt-8 md:pt-10 pb-20">
                            <button type="button" 
                                    className={`px-8 py-1 md:px-16 md:py-2 text-white ${button_color} hover:bg-stone-900 shadow-[6px_6px_12px_rgba(0,0,0,0.4)] text-lg md:text-2xl`} 
                                    style={{fontWeight: "500"}}
                                    onClick={() => { onAddToCart({ id, name, box_out, price, quantity, price_3, price_6, price_12 }), setOpenModal(true)}}>Buy now</button>
                            <Link to={`details/${id}`}>
                                <button type="button" className="border border-stone-800 px-4 py-1 md:px-12 md:py-2 text-stone-900 hover:bg-stone-100 shadow-[4px_4px_8px_rgba(0,0,0,0.2)] text-lg md:text-2xl">See details</button>
                            </Link>
                        </div>
                    </div>
                </div>
